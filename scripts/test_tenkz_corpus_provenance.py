    ...
    expected = Counter(
        {
            ...
            DimensionOwner.ROUTE: 244,
            DimensionOwner.LAYOUT: 191,
        }
    )
    if report.case_count != 435 or report.case_counts != expected:
        raise AssertionError(
...
"""
    ...
    _expect_dimension_failure(
        ...
        "case dimensions increased to 436",
    )
    _expect_dimension_failure(
        dataclasses.replace(report, cases=(*report.cases, extra_layout)),
        "composition/layout dimensions increased to 192",
    )
    for source, phrase in (
        ...
        (r"\tnjoin{a}{1mm}", "route/string dimensions increased to 245"),
    ):
