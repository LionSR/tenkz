...
(langs: grid, lattice, kernel); subsequent events carry an
...
Hard errors (exit 1):
  ...
  malformed-region     A lattice region lacks cells=, so its declarative
  ...
  empty-picture        A grid/lattice picture emitted no content
...
EMPTY_CHECK_LANGS = {"grid", "lattice", "kernel"}
                ...
                if e.kind not in allowed:
                    ...
                    if "cell" not in e.attrs:
                        self.adv("dialect-mismatch",
                                 ...
                                 f"cell=")
                elif e.kind == "region" and pic.lang == "lattice":
                    if "cells" not in e.attrs:
