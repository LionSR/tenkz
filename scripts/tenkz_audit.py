...
Hard errors (exit 1):
  ...
  label-overlap       A measured `tn label` rectangle strictly intersects a
                      measured glyph or visible-wire ink rectangle.
  bbox-coverage       A library-owned `tn label` use did not produce exactly
...
# `bbox`, `label-use`, `warning`, and `boundary` are cross-cutting/derived
...
class Picture:
    ...
    def content(self) -> list[Event]:
        ...
        return [e for e in self.events
                if e.kind not in {"bbox", "label-use", "boundary", "warning"}]
...
class Audit:
    ...
    def check_label_overlaps(self) -> None:
        """Reject strict intersections of emitted label and ink rectangles.

        Extents are integer scaled points measured from live TikZ anchors.
        Touching edges are allowed: they share a boundary but no area.
        """
        required = {"class", "id", "xmin", "xmax", "ymin", "ymax"}
        for pic in self.pictures:
            ...
            ink = [rect for rect in rectangles if rect[1] in {"glyph", "wire"}]
            for label in labels:
                for other in ink:
                    if (max(label[2], other[2]) < min(label[3], other[3])
                            and max(label[4], other[4]) < min(label[5], other[5])):
                        self.hard(
                            "label-overlap",
    ...
    def check_bbox_coverage(self) -> None:
        """Every library-owned label style use must yield one live bbox."""
        for pic in self.pictures:
