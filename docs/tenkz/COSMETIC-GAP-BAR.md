...
| `rmp-ii-mpu-blocking` | — | Promoted to `faithful` 2026-08-30 on an independent exact-hash 200/400dpi countersign: both factors, rails, physical contractions, open ends, a/b closures, labels, and ordering match. The house red capsule against the source's thin black circle is K2 colour/glyph/weight presentation. |
| `rmp-ii-mpu-two-shift` | — | Promoted to `faithful` 2026-08-30: the exact-hash two-viewer record already carries no defect; source pairing boxes, open legs, movers, and published palette match, while house-black row channels are K2 colour presentation. |
| `rmp-ii-mpu-normal-form` | — | Promoted to `faithful` 2026-09-06. Source-backed redraw on 2026-09-06 restores short dotted S-shaped paths with local free endpoints and both markers on the source-correct virtual legs in each panel. Marker beads explicitly carry no physical legs. Fresh author-source rendering and independent root and agent review confirm all four free string ends, marked-leg incidences, and one physical leg per PEPS star. Shared plane bearing and projected glyph shape are equivalent house presentation. |
...
| `rmp-ii-spectrum-fixed-points` | — | Promoted to `faithful` 2026-08-22 with the arc-route landing (#6201, Fable-verified): the two self-closures write `route=arc` and draw their protruding folds -- west of the first panel's factor, east of the second's -- restoring the closure ink the wave-1B migration lost; verified against the fig9-3 source block on the before/after pair. |
...
| `rmp-ii-staircase` | — | Repaired and promoted to `faithful` 2026-08-30 on a fresh exact-hash 200/400dpi two-viewer review: the ket-zero label now sits east of its dot as in the source and clears the outgoing diagonal bond; staircase orientation, four rounded V isometries, bonds, and physical openings remain intact. |
...
| `rmp-ii-boundary-lasso` | — | Promoted to `faithful` 2026-08-30 on a fresh exact-hash 200/400dpi two-viewer review: PR #245 made generated physical stubs inherit host species, so charge ink is source-red and flux ink declared gray. Equations, openings, factor order, labels, and contractions match; remaining glyph and typography differences are K2 house presentation. |
| `rmp-ii-boundary-region` | — | Promoted to `faithful` 2026-08-31 on a fresh exact-hash 200/400dpi two-viewer review: the existing public enclosure `tint` restores the source's translucent red region fill. Sheet, R and Rbar labels, reduced rail, seam, topology, and openings remain intact; the rounded contour is house presentation. |
...
| `rmp-ii-mpdo-ol` | — | Promoted to `faithful` 2026-08-30 on an independent exact-hash 200/400dpi countersign: two M tensors, elision, physical legs, virtual closure, and labels match. Horizontal orientation and the continuous rounded closure are K2 planar trace presentation. |
...
| `rmp-iii-a-ghz-state` | — | Promoted to `faithful` 2026-08-30 on an independent exact-hash 200/400dpi countersign against the authoritative source range: all 48 copy junctions, 62 internal Hadamard bonds, 20 virtual openings, and 48 physical legs match; spacing and stroke are K2 house presentation. |
...
| `rmp-iii-a-symmetry-sector` | — | Accepted as faithful by the maintainer on 2026-09-06. Both loop constructions and the outside R/X labels are satisfactory; no inscription or placement change is required. |
...
| `rmp-iii-b-r-tensor-right` | — | Accepted as faithful by the maintainer on 2026-09-06. The resolver and its outside label are satisfactory; no label-placement or contour change is required. |
...
| `rmp-iii-b-braid-three` | X | Partial case-only repair 2026-09-05 attaches all four resolver arms explicitly, including j and both parts of b. Retain structural-gap pending full projected-route and endpoint-ink fidelity; no kernel changes. |
| `rmp-iii-b-braid-four` | K3 | The 2026-09-06 label-placement correction moves the lower resolver name east to clear the string while preserving both resolver and anyon orders, the lower-rail bead and outgoing tail. Cosmetic residue remains: both resolver names are outside their white boxes rather than inscribed as in the source. |
| `rmp-iii-b-condensation` | K3 | Source-backed adjustment on 2026-09-06 adds collinear midpoint constraints to reduce the bowing between the three markers on each sheet. Both 6x4 sheets, all 40 perimeter openings and all 24 physical inter-sheet pairs remain intact. Cosmetic residue remains: the middle curves are not exactly taut and the shared bilayer basis joins sheets diagonally rather than by the source vertical drop. |
...
| `rmp-iv-ground-space-2d` | — | Promoted to `faithful` 2026-09-06. The 2026-09-06 physical-leg correction keeps the source short local out-of-plane ticks inside the nested region windows instead of extending each leg to the outer hull. Independent root source/render review confirms the two contours, east X, perimeter cuts, tensors and local physical legs; the focused 170-regression check passes. No directed-leg interpretation is introduced. |
...
| `rmp-iv-intersection-lhs-six` | — | Promoted to `faithful` 2026-08-29: the maintainer's countersign on the 110dpi plate (p.88) is the second-viewer countersign this row asked for. The first viewer flagged the L label outside the plane and corner-tick through-lines; the maintainer judged both presentational. |
| `rmp-iv-intersection-rhs-six` | — | Promoted to `faithful` 2026-08-29: the maintainer's countersign on the 110dpi plate (p.89) is the second-viewer countersign this row asked for. Same residue and same judgement as its left-hand twin. |
...
| `rmp-workbench-ii-newfig7` | — | Already `faithful` (2026-08-02 two-viewer review); two slots on every face keep the red and blue shift paths distinct and continuous through all three bricks, and the crossing-gadget styling is a house form. |
...
| `rmp-workbench-ii-positive-mpo-old` | — | Already `faithful` (2026-08-02 two-viewer review); both X branches meet the A and conjugate-A rails at the recorded equal X sizes, and their physical legs form one open vertical line. |
| `rmp-workbench-ii-peps-gauge-old` | — | Promoted to `faithful` 2026-09-06. Source-backed redraw on 2026-09-06 smooths the right-hand path through its three virtual-leg markers and removes inherited physical stubs from the marker beads. The lower blue endpoint exits southwest, separately from the black virtual tip. Fresh author-source comparison and independent root review confirm the one-to-three marked-leg relation, all open ends, and one physical leg on each PEPS star; remaining projection and label placement are equivalent house presentation. |
| `rmp-workbench-ii-peps-gauge-without-a` | — | Promoted to `faithful` 2026-09-06. Source-backed redraw on 2026-09-06 restores the source leg pairings: west and north on the left star, east and south on the right. Each marked path is one smooth open string and its beads have no physical stubs. Fresh author-source comparison and independent root review confirm both marked-leg incidences, open endpoints, and the single physical leg of each star. |
| `rmp-workbench-ii-mpu-wrap-second` | — | Promoted to `faithful` 2026-08-29 on maintainer countersign (p.99): the wrap reads upside down against the source, which the maintainer judged immaterial here; the stub pair touches without joining on both sides, the source legs being 0.5 long with rows 1 apart. |
| `rmp-workbench-ii-boundary-a-old` | — | Promoted to `faithful` 2026-08-31 on the same exact-hash 200/400dpi two-viewer enclosure-tint review: the existing public `tint` restores the archival panel's translucent red region. Archival R/R^c names, all-red partial-R rail, topology, and openings remain intact; the rounded contour and house-black interior sites are faithful K2 presentation. |
| `rmp-workbench-ii-boundary-b-old` | — | Promoted to `faithful` 2026-08-30 on a fresh exact-hash 200/400dpi two-viewer review: PR #245 restored host-species physical stubs, so charge ink is source-red and flux ink declared gray. Factor topology, openings, order, labels, contractions, and authoritative source palette match; remaining glyph differences are K2 house presentation. |
...
| `rmp-workbench-iii-eq51` | — | Promoted to `faithful` 2026-08-05: the closure ink landed, so the traced sides draw the source's returning rail below the word; the X ring, the three mpo atoms with both physical legs, and the elision dots all match Eq51.pdf. |
| `rmp-workbench-iii-eq52` | — | Promoted to `faithful` 2026-08-05 with the same closure ink as eq51: the traced sides draw the source's returning rail, and the X ring plus the one mpo atom with both physical indices open match Eq52.pdf. |
...
| `rmp-workbench-iii-diagram-two` | — | Promoted to `faithful` 2026-08-31: four glyphless pairing skins compact the return to the source's rounded proportions while preserving the direct Y--A bond and both open physical indices. Exact-hash 200/400dpi review confirms continuous joins, clear glyphs and annotation, and a clean audit. |
...
| `rmp-workbench-iii-historical-composite` | — | Accepted as faithful by the maintainer on 2026-09-06. The rectangular cycles, station order and directed half-edges are satisfactory; the previously recorded short-tick difference does not require a repair. |
...
| `rmp-workbench-iii-ghz-state-workbench` | — | Promoted to `faithful` 2026-08-30 with the published GHZ target: executable TeX, event logs, and 200/400dpi renders are identical, and the complete topology matches the authoritative source range. |
...
| `rmp-workbench-iii-g-injective-pull` | — | Promoted to `faithful` 2026-08-29 on maintainer countersign (p.121): two boxes north-west of the vertex left of the equals sign and two south-east of it right of the sign, same count and side as the source, which shears them into parallelograms. Follow-up closed 2026-08-29: the first viewer proposed routing this string orthogonally too, and the author panel settles it the other way -- the source string bows smoothly through its waypoints, so squaring it would move the case away from the source rather than toward it. The curve stays. Only rmp-iii-a-g-injective-projector, whose source really is a rounded-corner square, took route=orth. |
...
| `rmp-workbench-iii-mpo-injective-white` | — | Promoted to `faithful` 2026-08-29 on maintainer countersign (p.123): identical topology, labels, and equation. Follow-up closed 2026-08-29: the first viewer proposed routing this string orthogonally too, and the author panel settles it the other way -- the source string bows smoothly through its waypoints, so squaring it would move the case away from the source rather than toward it. The curve stays. Only rmp-iii-a-g-injective-projector, whose source really is a rounded-corner square, took route=orth. |
| `rmp-workbench-iii-mpo-on-peps-definition` | — | Promoted to `faithful` 2026-09-06. The 2026-09-06 compact plane redraw restores the projected MPO ring and its four skins alongside the separate five-leg PEPS star. All eight radial virtual openings remain explicit; the ring is red and has no invented central tensor. Root and independent source-render review confirm equivalent source geometry and incidence. The source equality is schematic: five left openings versus eight right openings, so the equation-sibling advisory remains visible. |
...
| `rmp-workbench-iii-eq59` | X | Moved to `structural-gap` 2026-08-29 on maintainer review (p.127): very wrong. The source draws a full lattice where the kernel panel reduces it to the single row and column that carry beads, and a torus where the kernel draws a circle with wrapped edges. Exotic; no kernel work directed. |
| `rmp-workbench-iii-g-injective-mpo` | — | Promoted to `faithful` 2026-08-30 on an independent exact-hash 200/400dpi countersign: four tensors, four internal bonds, and all eight labelled openings match with no crossing loss. The stock MPO box against the source circle is K2 house presentation. |
| `rmp-workbench-iii-peps-renormalization-one` | — | Promoted to `faithful` 2026-09-06. Source-backed redraw on 2026-09-06 restores eight open virtual half-indices meeting four edge-centred glyphless PEPS sites and four short angled physical legs. Ordinary index lines cross at the corners without invented over-under gaps or extra contractions. Fresh author-source comparison by the case agent and independent root render review confirm the twelve open ends and source incidence; isolated compilation, lint and audit pass. Remaining spacing and proportions are equivalent house presentation. |
| `rmp-workbench-iii-peps-renormalization-two` | — | Promoted to `faithful` 2026-08-30 under its exact-hash 200/400dpi two-viewer record: topology and angle-port legs are complete; the source hatch ticks denote blocked physical bundles, whose multiplicity owes no extra ink under LANGUAGE-1.0 section 5. |
