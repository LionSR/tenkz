...
| `rmp-iii-a-g-injective-projector` | — | Promoted to `faithful` 2026-08-29 on maintainer countersign of the 110dpi comparison plate (p.59): four L_g insertions, four ring tensors, one marked corner, and matching arrow directions on both sides; the rounded-rectangle L_g glyph against the source's circle is house style. Open follow-up, not a faithfulness gap: the red ring routes as a spline through its via stations where the author draws a rounded-corner square, so the loop reads less regular than the source. |
...
| `rmp-workbench-iii-mpo-injective-white` | — | Promoted to `faithful` 2026-08-29 on maintainer countersign (p.123): identical topology, labels, and equation. Open follow-up, not a faithfulness gap: the red route could be more regular than the spline it draws today. |
