...
| `rmp-app-toric-dual` | K1 | New 2026-08-05 with the kernel migration (wave 6): the paper dots the interior diagonals to mark the original lattice under the dual sheet, and the kernel wire grammar has no stroke-style key, so the recorded diagonals ink solid. Land a dashed spelling in the language, then re-review. |
