...
| `rmp-iii-a-g-injective-projector` | K2 | X resolved: the `wrong-source` pairing was repaired 2026-07-29, and the repaired case draws the paper projector, a normalized average of four cardinal L_g actions equal to the four-tensor red ring with the northwest black junction; the ledger names no residue. Re-review the hash-pinned render and promote to `faithful`. |
...
| `rmp-workbench-iii-g-injective-pull` | K2 | X resolved: the 2026-08-01 pairing re-audit verified the case against the author-source G-injective pull at lines 468-497, with the g string moving across the tensor and the northeast physical leg open; the ledger names no residue. Re-review the hash-pinned render and promote to `faithful`. |
