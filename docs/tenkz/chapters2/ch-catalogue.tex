...
\dbendpar The word does not bend the rail.  \tnval{arc} and
\tnval{straight} both leave the two ends joined by the segment between
them, so writing either in the picture above puts every line in the same
place, with one field of one record to tell the two sources apart.
\tnval{orth} draws that same segment here as well: the route is read
only where a string is rendered, so on the index wires above all three
words give one page.  Read \tnkey{route=}\tnval{arc} as the record of
which family a return belongs to rather than as the operation that
curves it, and reach for \tnval{orth} on the string below, which is
where the path does change.
