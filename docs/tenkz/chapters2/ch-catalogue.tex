...
tangent.  The object it draws is a network whose contraction pattern is a
...
\begin{tnexample}[
  formula={C^{\phantom{)}}_{\gamma}=\sum_{\alpha\beta}
    A_{\alpha}B_{\beta}},
...
would need a tangent-offset contract the language does not state; so do a
group-local frame and an atom-local one.
...
\begin{tnexample}[
  ...
  index={Left to right: dot, box, roundrect, ring, tri, triwest, dots,
    none, and the prelude's pill and mpo.  A skin says how a record is
    drawn and nothing about what it contracts.},
  signature={kernel-boundary|signature=}]
...
\begin{tenkz}[rows={wire}, cols=10, bonds=none]
  ...
  \tn[skin=dots]{} & \tn[skin=none]{N} & \tn[skin=pill]{P} &
  \tn[skin=mpo]{O}
\end{tenkz}
\end{tnexample}
...
A site that is not there is \tnkey{void=}.  The two words differ in what
survives: \tnval{open} keeps the site's incident indices and removes the
tensor, so the hole has a boundary; \tnval{sealed} removes the site and
its bonds together.
...
%   it; void=open would keep those indices and drop the tensor.
...
surviving sites.  What does \tnkey{void=}\tnval{open} write in the
signature instead?
...
and joins the two ends directly.  \tnval{arc} leaves and enters along its
ends' own faces, which is what closes a chain below itself.
...
The arc is not a decoration chosen for looks.  Each of the two returns
leaves an east face and enters a north face, and a straight segment
between those two points would arrive across the face rather than along
it.  The route states which family the path belongs to; the geometry
stage decides the curve.
...
\begin{tnexample}[
  formula={\lambda\ \text{inserted on one edge of the plaquette}},
...
The remaining wire keys decorate the rail rather than change its path.
\tnkey{stroke=} is the rail's pattern, and the corpus draws only three:
\tnval{solid} for a contraction, \tnval{dashed} for an index drawn but
not contracted, \tnval{dotted} for a lattice lying under the sheet being
drawn.  \tnkey{weight=} is strand multiplicity, and \tnkey{dir=} marks a
directed index against its dual.

\begin{tnexample}[
  wide,
  caption={the three strokes, a bundle, and a direction},
  index={Left: the three declared rail patterns.  Right: a bundled index
    drawn as three strands, carrying the direction mark that
    distinguishes a space from its dual.},
  signature={kernel-boundary|signature=}]
% Ink: stroke= is the rail's pattern; weight=bundle=n draws one
%   index as n strands; dir= marks the direction.
\begin{tenkz}[rows={wire}, cols=4, bonds=none]
...
the shape of the speech.  Four forms draw: \tnval{label} stands text at a
target, \tnval{bracket} measures a span from outside it, \tnval{enclosure}
strokes the offset hull of a selection, and \tnval{prose} records words in
place of a diagram.
...
Contours nest.  \tnkey{inset=} steps a second enclosure one clearance
outside the first, which is how a figure draws a boundary operator acting
on the open indices of a region it also has to show.

\begin{tnexample}[
  formula={\lvert\psi(X)\rangle},
  caption={a region, and an operator on its boundary},
  index={The inner contour is the retained block; the outer one, stepped
    one clearance out, is the operator acting on the indices the block
    leaves open.},
  signature={kernel-boundary|signature=phys:n, phys:n, phys:n, phys:n}]
% Ink: two contours over one selection; inset=1 steps the outer
%   one clear of the inner.
\begin{tenkz}[rows={wire}, cols=4, bonds=none, physical=up]
  ...
  \tnmark[form=enclosure, inset=1, label pos=e]{(1,1) .. (1,4)}{$X$}
\end{tenkz}
\end{tnexample}

\tnkey{slot=} tints a mark from the theme's semantic slots, and
\tnkey{tint} lays ink over the paper inside a contour rather than only
stroking it; the tutorial's blocked run shows the tinted form.

The registry's \tnkey{form=} list has four more words, and none of them
draws.  A source that asks for one is refused, with the code that says so:

\begin{tnrefusal}[
  caption={four declared forms that do not draw yet},
  index={The value parses --- it is in the registry --- and the render
    stage refuses it rather than omitting the ink silently.  The same
    answer comes back for \tnval{brace-below}, \tnval{cut}, and
    \tnval{band}.},
  diagnostic={[TKZ-KERNEL-RENDER-TODO] 'form=brace-above' does not draw
    yet.}]
% Refused: brace-above, brace-below, cut, and band are declared
% mark forms with no render.  Use bracket for a measured span
% and enclosure for a region.
\begin{tenkz}[rows={wire}, cols=3, west=open, east=open]
  \tn{A} & \tn{B} & \tn{C}
...
Read that refusal as the language's own rule, stated in
Chapter~\ref{ch:trouble}: unsupported requested ink is a coded error, never
a warning followed by omission.  A form that cannot be drawn is not
quietly dropped, so no figure ever ships missing a mark its source asked
for.
...
\begin{tnexample}[
  formula={M^{ij}=\sum_{\alpha}e\,(A^{i}\otimes\overline{A}^{j})\,e^{-1}},
  ...
  index={The two fusion atoms span both rows and combine the ket and bra
    ...
    physical indices $i$ and $j$.},
  signature={kernel-boundary|signature=open:e, open:w, phys:n, phys:s}]
% Formula: M^{ij} = e (A^i tensor conj(A)^j) e^{-1}; the fusers
%   combine the ket and bra virtual indices into one.
...
\begin{tnexample}[
  ...
  index={The upper row turns its north input east and its south input
    ...
    so every site drawn with it carries them.},
...
The atom class has a one-argument spelling, \tncmd{tndeclareatom}, for the
...
\dbendpar The two spellings do not accept the same port vocabulary.
\tncmd{tndeclareatom} takes the four named faces only ---
\tnval{west}, \tnval{east} typed \tnval{virtual}, and \tnval{up},
\tnval{down} typed \tnval{physical} --- and answers anything else with
\tnval{[TKZ-ATOM-INVALID-PORT]}.  The three-argument
\tncmd{tndeclare}\tnval{\{atom\}} also takes compass letters and numeric
angles, so a glyph with an off-axis face is declared there.
...
The benchmark and the continuous checks set the last one, which is why the
canonical cases are written in kernel words.
...
Two composition words close the alphabet.  \tncmd{tngroup} transforms a
sub-diagram as one object and takes \tnkey{frame=} alone: a group is a
sub-frame of its picture and shares its picture's metric, size class, and
audit, so those keys are refused at group scope rather than accepted and
left inert.  \tncmd{tnprose} records words where a diagram would stand,
for the step of an argument that is not drawn; on a checked relation it is
a hard error without a recorded opt-out, because prose is not a boundary
signature.

\subsection{Answers to the exercises}

\tnanswer{ex:cat-void} It writes the removed site's four virtual indices.
\tnval{void=open} keeps the incident indices and drops the tensor, so the
hole acquires a boundary: the four bonds that reached the centre become
open ends, and the signature gains four virtual entries beside the eight
physical legs.  \tnval{sealed} removes site and bonds together and adds
nothing.  A hole in a lattice is almost always the open one --- the
indices are what the hole is for.
