...
\section{Catalogue of Constructs}\label{ch:catalogue}
...
Identifying opposite sides of a square gives a torus.  A horizontal segment
and a vertical segment become closed cycles after this identification.  They
represent winding classes $(1,0)$ and $(0,1)$ and meet transversely once.

\begin{center}
\begin{tikzpicture}[x=1.3cm,y=1.3cm,>=Stealth]
  % Matching arrows identify opposite edges of this fundamental square.
  \draw[gray,dashed] (0,0) rectangle (3,3);
  \draw[gray,->] (0.35,0) -- (1.05,0);
  \draw[gray,->] (0.35,3) -- (1.05,3);
  \draw[gray,->] (0,0.35) -- (0,1.05);
  \draw[gray,->] (3,0.35) -- (3,1.05);
  \node[below] at (0.7,0) {$a$};
  \node[above] at (0.7,3) {$a$};
  \node[left] at (0,0.7) {$b$};
  \node[right] at (3,0.7) {$b$};
  \draw[red!75!black,thick,->] (0,1.5) -- (3,1.5);
  \draw[blue!75!black,thick,->] (1.5,0) -- (1.5,3);
  \node[above,red!75!black] at (2.3,1.5) {$c_x$};
  \node[right,blue!75!black] at (1.5,2.3) {$c_y$};
  \fill (1.5,1.5) circle[radius=1.4pt];
\end{tikzpicture}

\small
A fundamental square: identify equally labeled edges in the arrow direction.
The dot marks the intersection of $c_x$ and $c_y$, not a tensor insertion.
\end{center}

This topological schematic is drawn with TikZ.  The current \tnkey{wind=}
renderer uses a projected curve that does not reliably display the two
fundamental cycles; it should not be used as a verified drawing of this
construction.
...
and joins the two ends directly.  A rectangular periodic return uses
straight segments and four corner junctions, as below.  Each junction pairs
two virtual ports and carries no tensor.

\begin{tnexample}[
  wide,
  label={tnex:cat-periodic},
  formula={\lvert\psi(X)\rangle=\sum_{s_1\ldots s_N}
    \operatorname{tr}\!\left(XA^{s_1}\cdots A^{s_N}\right)\lvert s\rangle},
  caption={a periodic word with a symmetric return},
  index={The four corner junctions carry no tensor, and the elision cell
...
\begin{tenkz}[rows={wire,wire}, cols=9, bonds=none]
  ...
  \tn[at=(2,9), skin=none, name=se, ports={180:virtual,90:virtual}]{}
...
The corner junctions make the two sides of the return equal.  Every segment
is a contraction, so all virtual indices close through $X$ and only the three
physical indices remain open.

Use \tnval{arc} for a curved connection: it leaves and enters along its
ends' faces.  When neither end names a face, it follows the straight chord.
The next example uses four arcs to close a plaquette.
...
hull, \tnkey{route=}\tnval{\{all of z\}}, and a freer shape names its
waypoints with \tnkey{via=}.
...
Use \tnval{bracket} to mark a span.  Its \tnkey{label pos=} names the side:
\tnval{90} places it above and the default places it below.  Use
\tnval{enclosure} with \tnkey{tint} for a shaded region around a selection.

\begin{tnexample}[
  caption={brackets above and below, and a tinted enclosure},
  index={One bracket named on two sides, and one tinted enclosure.  A
    ...
    The enclosure's \tnkey{tint} shades the selected region.},
  signature={kernel-boundary|signature=}]
