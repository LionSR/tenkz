...
\begin{tnexample}[
  ...
  index={Left to right: dot, box, roundrect, ring, tri, triwest, dots,
    ...
    because \tnval{none} draws neither glyph nor label.  A skin says how
    a record is drawn and nothing about what it contracts.},
  signature={kernel-boundary|signature=}]
...
drawn.  \tnkey{weight=} says how many indices one drawn rail stands for, and
\tnkey{dir=} marks a directed index against its dual.

\begin{tnexample}[
  wide,
  caption={the three strokes, a bundle, and a direction},
  index={Left: the three declared rail patterns.  Right: one rail
    standing for three indices, carrying the direction mark that
    distinguishes a space from its dual.  The bundle is recorded, not
    drawn: the event stream reads \tnval{weight=bundle=3} and the page
    shows one line.},
  signature={kernel-boundary|signature=}]
...
the shape of the speech.  Of the eight words in the registry's list,
three put ink on the page: \tnval{label} stands text at a target,
\tnval{bracket} measures a span from outside it, and \tnval{enclosure}
strokes the offset hull of a selection.  \tnval{prose} is accepted and
recorded without ink, and the remaining four are refused.  This section
draws the three, then shows both of the other answers.
...
A group takes \tnkey{frame=} alone.  It is a sub-frame of its picture and
shares that picture's metric, size class, and audit, so \tnkey{size=},
\tnkey{metrics=}, and \tnkey{check=} are refused at group scope rather
...
as well.  The word \tnval{open} keeps the incident indices and drops only
the tensor, so the hole acquires a boundary: the four bonds that reached
the centre become open ends, and the signature gains four virtual entries
beside the eight physical legs.  \tnval{sealed} removes site and bonds
together and adds nothing.  A hole in a lattice is almost always the open
one, because the indices are what the hole is for.
