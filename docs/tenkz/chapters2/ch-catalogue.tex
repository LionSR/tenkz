...
\dbendpar The registry carries an \tnkey{inset=} key for this, named as
the nested-contour step.  It records a number in the event stream and
moves no ink: write one, write six, or leave the key out, and the two
contours above fall in the same places.  The 1.0 inventory lists it among
the spellings that go with nothing in their place, on exactly that
ground, so read it in the generated tables as a word on its way out
rather than as the operation that steps a contour.
...
them; the only sources that turn \tnval{strict} on are the five refusal
cases that prove it refuses, the one below among them.
