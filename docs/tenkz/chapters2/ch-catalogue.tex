...
\dbendpar The word does not bend the rail.  Of the three path shapes only
\tnval{orth} reaches the ink; \tnval{arc} and \tnval{straight} both leave
the two ends joined by the segment between them, so writing either in the
picture above puts every line in the same place, with one field of one
record to tell the two sources apart.  Read \tnkey{route=}\tnval{arc} as
the record of which family a return belongs to rather than as the
operation that curves it, and reach for \tnval{orth} below when the path
...
blocked run shows the tinted form.
