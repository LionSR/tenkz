...
% has matching boundary signatures on its two sides, including the zipper
% with its face-specific splitting endpoint.
...
The face spelling makes the arity explicit: \tnkey{west at=center,
east at=\{1,2\}} is the splitting orientation, with one fused bond on
the west and two separate bonds on the east.  Reversing the two face
declarations gives the fusion orientation.  The zipper in
Example~\ref{ex:worked-zipper} uses the splitting form at the end of a
word, so its fused face may attach inside the same picture.
