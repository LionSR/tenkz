...
\texttt{north=}, \texttt{south=} &
  \texttt{open|none|cup|cup=}\meta{m} & --- &
  override one virtual side; \texttt{cup} contracts matching ket--bra
  boundary indices \\
\texttt{sheets=} & \meta{n}\texttt{|\{ket,bra,\ldots\}} & \texttt{1} &
  numeric sheets or a top-to-bottom role stack \\
\texttt{outer legs=} & \texttt{both|top|bottom|none} & --- &
