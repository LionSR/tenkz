...
A side exposes indices only when it says so.  A picture that states no side
policy draws no side leg and carries the empty boundary signature on those
sides, so a map keeps its ends by writing \tnkey{boundary=open} -- or
\tnkey{west=open} for one side alone -- and \tnkey{boundary=none} is the
explicit spelling of what the picture already does.  The frame's grid places
atoms; it does not claim that every rim cell owns a dangling index, which is
why a circuit pyramid or a single four-legged junction collects no legs it
did not ask for.

The rest of the alphabet: use
...
A bracket mark over a cell range (\tncmd{tnmark} with
\tnkey{form=bracket}) declares a brace or measured group over existing
atoms; \tnval{skin=dots} declares an ellipsis cell whose wires pass
through.  Neither creates a contraction.  A wide atom is appropriate only
when the formula has already replaced the run by one blocked tensor.
...
spelling.  Inline composition belongs at composition scope, not in a key:
no record inside the picture acquires an ``inline'' property.
