...
picture. \\
\addlinespace
\emph{\tnval{trace=physical} requires centred physical faces} &
The per-site arc closes one upper port into one lower port.  An
asymmetric or multi-port face remains open; use a centred face or draw
the intended multi-index contraction as a paired stack. \\
\addlinespace
...
\begin{tnmultiples}[
    ...
    caption   = {wider gaps restore the label band},
    index     = {Both rails point down, so the upper labels descend into
                 the gap.  The stock gap violates the overlap audit.
                 \tnval{layer sep=1.5} is sufficient;
                 \tnval{layer sep=2} doubles the gap.  The east cup is
                 $\rho^R$; the west pair remains open.},
    variants  = {{layer sep=1.5}{layer sep=1.5},
                 {layer sep=2}{layer sep=2}},
    label     = {ex:layer-sep},
  ]
% Both rails point down; the stock gap violates the overlap audit.
% These panels show safe local repairs at 1.5 and 2.
