...
| TNMPDOFusionTracePower | ch21_mpdo_rfp_fusion_isometries:520 | `[rows={op,op}, periodic]` (both layers traced, above+below): $M_\alpha$ row over $M_\beta$ row `= \sum_\gamma` $\chi$ row over $M_\gamma$ row | trivial (two-row periodic closes above and below) |
| TNMPDORecursiveStructureOperator | ch21_mpdo_rfp_fusion_isometries:610 | `Q=` `[physical=updown]: \tn[mpo]{X}&\tn[mpo]{X}&\tn[mpo]{X}&\tnskip&\tn[mpo]{X}` + $M_\gamma$ trace cell at the east end | needs-judgment (tail trace-cell loop; candidate `\tnpic[periodic]{\tnX{M_\gamma}}` factor) |
