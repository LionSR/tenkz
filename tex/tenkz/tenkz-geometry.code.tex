...
\cs_new_protected:Npn \__tenkz_geom_require_kind:nnn #1#2#3
  {
    ...
    \str_if_eq:VnF \l__tenkz_geom_tmp_tl {#2}
      {
        \msg_error:nnnnn {tenkz}{geom-frame-kind} {#1}
...
% units.  Families: circle (hx = radius), rect (half-extents), roundrect
% (rect shrunk by the house corner radius, then re-inflated), triangle
% (isoceles, apex east: vertex max is exact for polygons).
\cs_new:Npn \__tenkz_geom_support:nnnnn #1#2#3#4#5
  {
    \str_case:nnF {#1}
      {
        {circle} { (#2) }
        {rect}
          { abs( (#2) * cosd((#5)-(#4)) ) + abs( (#3) * sind((#5)-(#4)) ) }
        {roundrect}
          {
            abs( ( (#2) - \__tenkz_geom_rr:nn{#2}{#3} ) * cosd((#5)-(#4)) )
            + abs( ( (#3) - \__tenkz_geom_rr:nn{#2}{#3} ) * sind((#5)-(#4)) )
            + \__tenkz_geom_rr:nn{#2}{#3}
          }
        {triangle}
          {
            max(
              (#2) * cosd((#5)-(#4)) ,
              - (#2) * cosd((#5)-(#4)) + (#3) * sind((#5)-(#4)) ,
              - (#2) * cosd((#5)-(#4)) - (#3) * sind((#5)-(#4)) )
          }
      }
      { \msg_expandable_error:nnn {tenkz}{geom-family} {#1} 0 }
  }
\cs_new:Npn \__tenkz_geom_rr:nn #1#2
  { min( \__tenkz_metric_ratio:n {corner} , (#1) , (#2) ) }

% fold max-support over a set of {family}{hx}{hy}{theta}{cx}{cy} records:
% each contributes its centre's reach plus its shape's support
\cs_new_protected:Npn \__tenkz_geom_support_max:nnN #1#2#3
  {
    \tl_set:Nn #3 { -10000 }
    \seq_map_inline:cn {#1}
      { \__tenkz_geom_support_max_one:nnnnnnnN ##1 {#2} #3 }
  }
\cs_new_protected:Npn \__tenkz_geom_support_max_one:nnnnnnnN #1#2#3#4#5#6#7#8
  {
    \tl_set:Ne #8
      { \fp_eval:n
          { max( #8 ,
                 (#5) * cosd(#7) + (#6) * sind(#7)
                 + \__tenkz_geom_support:nnnnn {#1}{#2}{#3}{#4}{#7} ) } }
  }
