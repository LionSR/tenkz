...
% A map label is `auto', hence normally offset from the horizontal path.  Only
% split the measured baseline when the opaque label box actually covers the
% stroke; otherwise the complete source-to-target segment remains visible.
\cs_new_protected:Npn \tenkz_cd_emit_map_wire_bboxes:n #1
  {
    ...
    \dim_set:Nn \l__tenkzcd_mapwire_ymax_dim
      {\l__tenkzcd_mapwire_y_dim + \l__tenkzcd_mapwire_half_dim}
    \pgfextracty{\l__tenkzcd_maplabel_ymin_dim}
      {\pgfpointanchor{tenkz-map-label-\the\tenkz@pictureid-#1}{south}}
    \pgfextracty{\l__tenkzcd_maplabel_ymax_dim}
      {\pgfpointanchor{tenkz-map-label-\the\tenkz@pictureid-#1}{north}}
    \dim_compare:nNnTF {\l__tenkzcd_maplabel_ymax_dim} >
      {\l__tenkzcd_mapwire_ymin_dim}
      {
        \dim_compare:nNnTF {\l__tenkzcd_maplabel_ymin_dim} <
          ...
          {
            \coordinate (tenkz-map-left-cut-\the\tenkz@pictureid-#1) at
              (tenkz-map-label-\the\tenkz@pictureid-#1.west |-
               tenkzmap-\int_use:N\l__tenkzcd_fromrow_int-
                 \int_use:N\l__tenkzcd_fromcol_int.base~east);
            \coordinate (tenkz-map-right-cut-\the\tenkz@pictureid-#1) at
              (tenkz-map-label-\the\tenkz@pictureid-#1.east |-
               tenkzmap-\int_use:N\l__tenkzcd_fromrow_int-
                 \int_use:N\l__tenkzcd_fromcol_int.base~east);
            \begingroup
              \tenkz@begininkuse{wire}
