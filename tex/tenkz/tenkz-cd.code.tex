...
\dim_new:N \l__tenkzcd_mapwire_half_dim
\dim_new:N \l__tenkzcd_mapwire_y_dim
\dim_new:N \l__tenkzcd_mapwire_ymin_dim
\dim_new:N \l__tenkzcd_mapwire_ymax_dim
    ...
    \bool_if:NT \l__tenkzcd_mapvalid_bool
      {
        ...
        \tenkz_cd_emit_map_bboxes:nn {#7}{#4}
...
% the pair of segments ending at the label's west/east edges.  A fused bond's
% outer half-width is 1.1pt (0.55pt line width around a 1.1pt double gap).
\cs_new_protected:Npn \tenkz_cd_emit_map_bboxes:nn #1#2
  {
    \tenkz@bboxlabeluse
    ...
    \str_if_eq:nnTF {#2}{1}
      { \dim_set:Nn \l__tenkzcd_mapwire_half_dim {1.1pt} }
      { \dim_set:Nn \l__tenkzcd_mapwire_half_dim {\tenkz@wirewidth / 2} }
    \tenkz_cd_emit_map_wire_bboxes:n {#1}
    \tenkz@cleanupstoredlabelgeometry{map-\the\tenkz@pictureid-#1}
  }

\cs_new_protected:Npn \tenkz_cd_emit_full_map_wire:
  {
    \begingroup
      ...
      \tenkz@emitwirebbox
        ...
        {tenkzmap-\int_use:N\l__tenkzcd_torow_int-
          \int_use:N\l__tenkzcd_tocol_int}{base~west}
        {\l__tenkzcd_mapwire_half_dim}
    \endgroup
  }

% A map label is `auto', hence normally offset from the horizontal path.  The
% same normalized visible rectangle emitted for the label decides whether its
% opaque fill/stroke overpaints the wire and supplies both cut coordinates.
% Otherwise the complete source-to-target segment remains visible.
\cs_new_protected:Npn \tenkz_cd_emit_map_wire_bboxes:n #1
  {
    \pgfextracty{\l__tenkzcd_mapwire_y_dim}
      {\pgfpointanchor
        {tenkzmap-\int_use:N\l__tenkzcd_fromrow_int-
          \int_use:N\l__tenkzcd_fromcol_int}{base~east}}
    \dim_set:Nn \l__tenkzcd_mapwire_ymin_dim
      {\l__tenkzcd_mapwire_y_dim - \l__tenkzcd_mapwire_half_dim}
    \dim_set:Nn \l__tenkzcd_mapwire_ymax_dim
      {\l__tenkzcd_mapwire_y_dim + \l__tenkzcd_mapwire_half_dim}
    ...
    \dim_compare:nNnTF {\l__tenkzcd_maplabel_ymax_dim} >
      {\l__tenkzcd_mapwire_ymin_dim}
      {
        \dim_compare:nNnTF {\l__tenkzcd_maplabel_ymin_dim} <
          {\l__tenkzcd_mapwire_ymax_dim}
          {
            \tenkz@canvascoordinate
              {tenkz-map-left-cut-\the\tenkz@pictureid-#1}
              {\dim_use:N\l__tenkzcd_maplabel_xmin_dim}
              {\dim_use:N\l__tenkzcd_mapwire_y_dim}
            \tenkz@canvascoordinate
              {tenkz-map-right-cut-\the\tenkz@pictureid-#1}
              {\dim_use:N\l__tenkzcd_maplabel_xmax_dim}
              {\dim_use:N\l__tenkzcd_mapwire_y_dim}
            \begingroup
              \tenkz@begininkuse{wire}
              \tenkz@emitwirebbox
                {tenkzmap-\int_use:N\l__tenkzcd_fromrow_int-
                  \int_use:N\l__tenkzcd_fromcol_int}{base~east}
                {tenkz-map-left-cut-\the\tenkz@pictureid-#1}{center}
                {\l__tenkzcd_mapwire_half_dim}
            \endgroup
            \begingroup
              \tenkz@begininkuse{wire}
              \tenkz@emitwirebbox
                {tenkz-map-right-cut-\the\tenkz@pictureid-#1}{center}
                {tenkzmap-\int_use:N\l__tenkzcd_torow_int-
                  \int_use:N\l__tenkzcd_tocol_int}{base~west}
                {\l__tenkzcd_mapwire_half_dim}
            \endgroup
          }
          { \tenkz_cd_emit_full_map_wire: }
      }
      { \tenkz_cd_emit_full_map_wire: }
  }
