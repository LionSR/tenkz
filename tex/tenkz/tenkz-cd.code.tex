...
\cs_new_protected:Npn \tenkz_cd_emit_map_bboxes:nn #1#2
  {
    ...
    \tenkz@emitmanualglyphbbox
      {tenkzmap-\int_use:N\l__tenkzcd_torow_int-
        \int_use:N\l__tenkzcd_tocol_int}
    \str_if_eq:nnTF {#2}{1}
      { \dim_set:Nn \l_tmpa_dim {1.1pt} }
      { \dim_set:Nn \l_tmpa_dim {\tenkz@wirewidth / 2} }
    \begingroup
          ...
          \begin{scope}
            \matrix (tenkzmap)
              [matrix~of~math~nodes,
               ...
               % Typed-map objects are measured after the matrix is built.
               % Their final PGF shape remains customizable, but keeping the
               % path corners sharp makes that later measurement exact.
               nodes={sharp~corners}]
