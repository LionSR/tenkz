...
% dedup): #1 is a short namespace tag so the four call families never
% collide in the one key space, #2 the site's own row/col key.  The
% picture-wide reset lives once at the top of \tenkz_draw_legs: -- every
% consumer runs inside that one pass, after it, so a second reset before
% \tenkz_emit_signature: would only ever find its own namespace's keys.
...
\cs_new_protected:Npn \tenkz_draw_lower_surplus:nnn #1#2#3
  {
    ...
    \tenkz_once_p:nnT {ls} {#1-\l__tenkz_pwlowbase_tl}
      {
        \tenkz_get:NeN \l__tenkz_name_prop
          ...
          { \int_eval:n{#1+1} - \l__tenkz_pwbase_tl } \l__tenkz_pb_tl
          {
            ...
            \bool_if:NTF \l_tmpa_bool
              ...
              }
              {
                ...
                \str_if_eq:VnTF \l__tenkz_faceports_tl {center}
                  { \tenkz_once_p:nnT {pwc} {#1-\tl_use:N\l__tenkz_pwbase_tl}
...
\cs_new_protected:Npn \tenkz_pair_below_wire:nn #1#2
  {
    ...
    \str_if_eq:VnTF \l__tenkz_faceports_tl {center}
      {
        \tenkz_once_p:nnTF {pwc} {#1-\tl_use:N\l__tenkz_pwbase_tl}
          {
            \draw[physical~leg]
    ...
    \int_step_inline:nn {\l__tenkz_openlowerspan_int}
      {
        ...
        \tenkz_once_p:nnT {olo} {#1-\l__tenkz_pwbase_tl}
...
% edge like any other leg.  Row #1 feeds the same active-face predicate
% the drawing and audit passes use, instead of re-testing the up/updown,
% down/updown precedence against \l__tenkz_cphys_tl by hand.
            ...
            \bool_if:NF \l__tenkz_cnolegs_bool
              {
                \tenkz_face_ports:nN {up} \l__tenkz_faceports_tl
                \str_if_eq:VnTF \l__tenkz_faceports_tl {center}
                  { \tenkz_once_p:nnT {sol} {#1-\tl_use:N\l__tenkz_pwbase_tl}
...
\cs_new_protected:Npn \tenkz_emit_signature:
  {
    ...
    % the `sol' namespace of \l__tenkz_once_prop resets with the rest of
