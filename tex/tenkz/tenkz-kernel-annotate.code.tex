...
%   * every other endpoint resolves its owner and the next route point, then
%     reads that page-space bearing in the atom's complete carrier basis;
%   * string waypoints do the same for both incident segments, cyclically for
%     a closed route, while a generated hull departure samples its frozen path;
...
% bead-tangent continuation of issue #15 is what empties the remaining ledger.
...
% chooser, just like an omitted field; an atom in the blind ledger receives a
...
\cs_new_protected:Npn \__tenkz_kernel_r_label_mark_bearing:nnn #1#2#3
  {
    \__tenkz_kernel_r_local_vector:nnnNN {#1}
    ...
    \quark_if_no_value:NF \l__tenkz_kernel_lab_host_tl
      {
        ...
        % The authored port face remains authoritative.  A circle chord may
        % cross the label band again away from that port, so this host cannot
        % make a verified free-face promise until chord/port occupancy has an
        % explicit contract of its own.
        \str_if_eq:eeT { \__tenkz_kernel_r_frame_word: } {circle}
          { \__tenkz_kernel_r_label_blind:V \l__tenkz_kernel_lab_host_tl }
      }
  }
...
% A hull route owns generated points absent from the authored address chain,
% so that one case samples the already-frozen route beside the endpoint.
            ...
            \quark_if_no_value:NF \l__tenkz_kernel_lab_atom_tl
              {
                ...
                \tl_if_empty:NTF \l__tenkz_kernel_lab_face_tl
                  ...
                  {
                    \__tenkz_kernel_r_label_hull_bearing:nnV
...
% A generated hull lane has no authored neighbour address.  Its frozen path
% is already available here, so sample just inside the appropriate endpoint.
\cs_new_protected:Npn \__tenkz_kernel_r_label_hull_bearing:nnn #1#2#3
  {
    ...
    \__tenkz_string_inked:VTF \l__tenkz_kernel_lab_owner_tl
      {
            ...
            \fp_eval:n
              {
                \str_if_eq:nnTF {#2} {first}
                  ...
                  { 1 - \c__tenkz_kernel_lab_route_sample_fp }
              }
          }
        \exp_args:NVV \__tenkz_kernel_r_label_route_point:nnNN
          ...
          \l__tenkz_kernel_lab_next_x_fp
          \l__tenkz_kernel_lab_next_y_fp
        \__tenkz_kernel_r_label_mark_bearing:nnn {#3}
          { \l__tenkz_kernel_lab_next_x_fp }
          { \l__tenkz_kernel_lab_next_y_fp }
      }
      { \__tenkz_kernel_r_label_blind:n {#3} }
  }
\cs_generate_variant:Nn \__tenkz_kernel_r_label_hull_bearing:nnn { nnV }
...
% no endpoints at all.  Walk the canonical authored address chain once and
% mark both incident bearings at each waypoint that owns an atom.  Generated
% path controls are deliberately absent: they are rendering detail, not
% semantic incidences.
                ...
                \int_compare:nNnT
                  { \seq_count:N \l__tenkz_kernel_lab_route_seq } > {1}
                  {
                    \int_step_inline:nn
                      ...
                      {
                        \__tenkz_kernel_r_label_waypoint:nnn
                          {##1}
                          {
...
\cs_new_protected:Npn \__tenkz_kernel_r_label_waypoint:nnn #1#2#3
  {
...
% issue #15 answers it in the bead-tangent step.
