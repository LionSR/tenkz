...
%     closed route, while a generated hull reads its exact endpoint tangent;
...
% is already available here, so read its exact endpoint tangent.  This cannot
% skip a short first/last segment as a fixed arclength fraction could.
...
% A segment joining two stations on the same atom lies under that atom's
% glyph.  It cannot establish a free label face, so suppress just that side
% of the waypoint scan and put the owner in the conservative blind ledger.
\cs_new_protected:Npn \__tenkz_kernel_r_label_waypoint_neighbor:nN #1#2
  {
    ...
    \exp_args:NV \__tenkz_kernel_r_label_end_atom:nNNNN
      ...
      \l__tenkz_kernel_lab_far_col_tl
      \l__tenkz_kernel_lab_far_bool
    \quark_if_no_value:NF \l__tenkz_kernel_lab_far_atom_tl
      {
