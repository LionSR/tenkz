...
\seq_new:N \g__tenkz_language_tombstones_seq
...
% A tombstone row is the one record of a deleted spelling (LANGUAGE-1.0
% section 10).  Unlike every other registry row it is read at run time and
% not only by the tooling: the kernel installs the refusals from this
% sequence, so the words a parser rejects and the migration it prints cannot
% drift from the words the reference and the linter publish.
\cs_new_protected:Npn \__tenkz_language_registry_tombstone:nnnnn #1#2#3#4#5
  { \seq_gput_right:Nn \g__tenkz_language_tombstones_seq {{#1}{#2}{#3}{#4}{#5}} }
...
% \tnset is bound by \tenkzkernel; since the S4 surface swap tenkz.sty makes
