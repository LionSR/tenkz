...
% one corner label: semantic corner #1, placed in the cell-corner pocket picked
% by (col-extreme reg #2, x-sign #3, row-extreme reg #4, y-sign #5); text #6.
% A cell corner lies half one lattice pitch from the extreme member site in
% both logical directions: it is the Voronoi point maximally separated from
% that site and any diagonal nonmember.  Map that point once with the lattice,
% then center the upright label there.  Its paper fill may interrupt the region
% stroke, while the node stays clear of both site glyphs.
\cs_new_protected:Npn \tenkzl_corner_label:nnnnnn #1#2#3#4#5#6
  {
    \tenkzl_map:nn
      { \tenkzl_cx:n{#2} #3 \tenkz@r@latticepitch\tenkz@pitch / 2 }
      { \tenkzl_cy:n{#4} #5 \tenkz@r@latticepitch\tenkz@pitch / 2 }
    \node[tn~label, anchor=center, fill=tenkzPaper]
      (tenkzl-region-label-probe) at
      ( \dim_use:N \l__tenkzl_px_dim , \dim_use:N \l__tenkzl_py_dim )
      { $#6$ };
    \tenkzl_record_obstacle_bbox:n {tenkzl-region-label-probe}
  }
% place the label in the requested corner pocket of the boundary box; center
% stays at the box midpoint.  An unrecognized `label at` draws nothing, as
% before.
