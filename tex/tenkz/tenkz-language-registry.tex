...
%
% Everything above that reads a row is the tombstone ledger's own (issue
% #6187), and none of it is written a second time here.  Until it lands the
% rows below are inert: the registrar discards them and no tool parses one,
% and what refuses a spelling is the parser branch beside its row.  The rows
% and the ledger have to arrive in that order, the ledger first, or the tree
% carries two records with nothing holding them together.
...
% before the registry could hold them; the disposition audit will read its
% list of dead commands from here once it can read a row at all.
