...
% tenkz environment, \tnpic, and tenkzlattice.  These environments
...
% read it as their default skin at option-parse time;
...
% a TikZ node whose bounding box is the member's actual rendered ink, so
% external labels and enclosed paths are measured rather than budgeted.
% Grid cells register their glyph nodes.  The renderer adds only the
% existing daylight constant.
...
\msg_new:nnn { tenkz } { enclosure-unknown }
  {
    Unknown~enclosure~member~'#1'.~Name~a~rendered~cell~or~an~earlier~
...
\msg_new:nnn { tenkz } { region-context }
  {
    \token_to_str:N \tnregion~is~available~inside~tenkzlattice,~not~
    the~'#1'~dialect.
  }
...
% `\tnregion' is one grammatical class.  The public command dispatches to
% the active picture rather than assuming a lattice scope.
