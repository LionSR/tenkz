...
% tenkz environment and \tnpic.  These environments group, so a
% picture-local assignment scopes itself.  Explicit per-cell
...
% species.  Every picture-scope sub-language (and the command-scope
% \tntree family) hand-copied this 4-6 line block, two of them spelling
% the forward two different ways (\pgfkeys{/tenkz/pitch=#1} vs
% \pgfqkeys{/tenkz}{pitch=#1}); one mechanism, one spelling (GOAL.md
% "one universal language"). Per-atom families (/tenkz/cell) do NOT
% call this: pitch/compact/inline pick the picture's metric once, not
% per atom, so those families never had these keys and do not gain
% them here -- an existing pattern, not a new exemption.
