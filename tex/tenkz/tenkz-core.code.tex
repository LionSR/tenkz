...
% run.  A semantic skin may be restyled from (say) circle to rectangle; the
% event must describe that live node, not the base style's starting shape.
% PGF does not retain `rounded corners' as part of the node-shape identity: both
% sharp and rounded nodes are named `rectangle'.  Therefore a built-in
% roundrect retains its declared production radius.  Later corner-radius
% overrides are outside the audited customization contract; use a box skin for
% a sharp rectangle rather than mutating a pill/ring's rounded-corner path.
\def\tenkz@resolveglyphshape#1#2{%
  ...
  \def\tenkz@declaredroundrect{roundrect}%
  \def\tenkz@declaredshape{#2}%
  ...
  \else\ifx\tenkz@livepgfshape\tenkz@pgfrectangle
    \ifx\tenkz@declaredshape\tenkz@declaredroundrect
...
\def\tenkz@captureglyphgeometry#1#2{%
  \iftenkz@bboxcapture
    \ifnum\tenkz@pictureid>0\relax
      \global\advance\tenkz@inkuid by 1\relax
      \tenkz@inkowner=\tenkz@inkuid
      \tenkz@glyphradius=#2\relax
      \tenkz@resolveglyphshape{\tikzlastnode}{#1}%
...
\tikzset{
  ...
  tenkz audited glyph/.style 2 args={
    append after command={\pgfextra{\tenkz@captureglyphgeometry{#1}{#2}}}},
}
