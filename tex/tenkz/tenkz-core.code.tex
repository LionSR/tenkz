...
\tikzset{
  ...
  box tensor/.style={tenkz audited glyph, draw=tenkzInk, fill=tenkzPaper,
      rectangle,
...
% Node rectangles use live TikZ anchors.  Horizontal wire rectangles use the
% two live endpoint anchors plus the stroke's vertical half-width; both caps
% terminate under opaque nodes, so longitudinal cap ink is not visible.
...
\def\tenkz@capturelabelbbox{%
  \iftenkz@bboxcapture
    ...
    \fi
  \fi}
\def\tenkz@captureglyphbbox{%
  \iftenkz@bboxcapture
    \ifnum\tenkz@pictureid>0\relax
      \tenkz@emitnodebbox{glyph}{\tikzlastnode}%
    \fi
  \fi}
...
\tikzset{
  ...
  tenkz audited glyph/.style={
    append after command={\pgfextra{\tenkz@captureglyphbbox}}},
}
