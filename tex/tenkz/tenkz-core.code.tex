...
\tikzset{
  ...
  tn label/.style={tenkz audited label, shape=rectangle, text=tenkzInk,
      fill=tenkzPaper, inner sep=1pt, font=\scriptsize},
...
% Labels use visible live rectangles.  Core glyph nodes retain their visible
% filled shape after outer-separation removal: circles use live cardinal
...
\newcount\tenkz@auditpendingdepth
...
\def\tenkz@armvisiblesnapshot#1{%
  \global\advance\tenkz@auditpendingdepth by 1\relax
  \expandafter\xdef\csname tenkz@auditpending@%
    \the\tenkz@auditpendingdepth\endcsname{#1}}
...
% The unarmed branch is the original four-argument call.  An audited token
% wraps only its node's one real outer-adjust hook and its original path-use
% callback; all arguments and backend calls are forwarded unchanged.
\let\tenkz@multipartnode\pgfmultipartnode
\def\pgfmultipartnode#1#2#3#4{%
  \ifnum\tenkz@auditpendingdepth=0\relax
    \tenkz@multipartnode{#1}{#2}{#3}{#4}%
  \else
    \edef\tenkz@token{\csname tenkz@auditpending@%
      \the\tenkz@auditpendingdepth\endcsname}%
    \global\expandafter\let\csname tenkz@auditpending@%
      \the\tenkz@auditpendingdepth\endcsname\relax
    \global\advance\tenkz@auditpendingdepth by -1\relax
...
\def\tenkz@visiblestateerror#1#2#3{%
  ...
  \or\PackageError{tenkz}{Audited #2 node '#3' has no active fill}%
      {Audited labels require an opaque rectangular background fill.}%
...
\def\tenkz@capturelabelbbox#1{%
  ...
  % Exact label geometry is the filled rectangle.  Without that background,
  % the support would be the individual text glyphs, which this audit does not
  % attempt to reconstruct.
  \ifnum\tenkz@visibleinvalid=0\relax
    \ifnum\csname tenkz@fill@#1\endcsname=1\relax\else
  ...
  \ifnum\tenkz@visibleinvalid=0\relax
    \ifnum\tenkz@pictureid>0\relax
      ...
      \iftenkz@labelrectangle
        ...
        \pgfextractx{\tenkz@bboxxmin}{\pgfpointanchor{\tikzlastnode}{west}}%
        \pgfextractx{\tenkz@bboxxmax}{\pgfpointanchor{\tikzlastnode}{east}}%
        \pgfextracty{\tenkz@bboxymin}{\pgfpointanchor{\tikzlastnode}{south}}%
        \pgfextracty{\tenkz@bboxymax}{\pgfpointanchor{\tikzlastnode}{north}}%
...
\def\tenkz@installlabelaudit{%
  ...
  \ifnum\tenkz@pictureid>0\relax
    ...
    \edef\tenkz@installlabelsnapshot{%
      \noexpand\tikzset{%
        execute at end node={%
          \noexpand\tenkz@snapshotvisible{\tenkz@glyphsnaptoken}},%
