...
\cs_new_protected:Npn \__tenkz_kernel_check_off:n #1
  {
    ...
    \int_compare:nNnTF { \seq_count:N \l__tenkz_kernel_match_seq } < {2}
      { \msg_error:nnn {tenkz}{kernel-check-off} {#1} }
      {
        ...
        % The reason is recorded as the author spelled it, not as it means:
        % a record carrying a macro would be checked as one thing and
        % written as another, and what a reader wants back is the sentence
        % on the page.  Reducing it to characters here makes the checks
        % below read exactly what the stream will carry.
        \tl_set:Ne \l__tenkz_kernel_check_reason_tl
