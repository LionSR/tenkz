...
\cs_new_protected:Npn \__tenkz_kernel_begin:n #1
  {
    ...
    % One equation, one pitch.  A panel that named a profile of its own would
    % scale one side of an asserted equality and not the other, so inside an
    % equation the profile belongs to the equation and is refused on a panel.
    % The refusal acts on the staged keys, before they become the picture
    % record: a key the kernel refuses must not reach the stream either, or a
    % reader is told of a pitch the picture was never drawn at.
    \bool_if:NT \l__tenkz_kernel_ineq_bool
      {
        \prop_if_in:NnT \l__tenkz_kernel_stage_prop {metrics}
          {
            \prop_if_in:NnF \l__tenkz_kernel_eq_policy_prop {metrics}
              {
    ...
    \int_compare:nNnTF { \seq_count:N \l__tenkz_kernel_match_seq } < {2}
      { \msg_error:nnn {tenkz}{kernel-check-off} {#1} }
      {
        ...
        \tl_set:Ne \l__tenkz_kernel_check_reason_tl
          { \seq_use:Nn \l__tenkz_kernel_match_seq { \c_colon_str } }
      ...
      }
      {
        \regex_match:nVTF { \| } \l__tenkz_kernel_check_reason_tl
          ...
          }
          {
            ...
            \seq_put_right:Ne \l__tenkz_kernel_check_reason_seq
              { \tl_use:N \l__tenkz_kernel_check_reason_tl }
          }
      }
  }
