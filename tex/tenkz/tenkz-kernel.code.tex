...
% compass word rather than as a node, so no cell says where the ink goes.
% The word itself does: the stub leaves the authored end by that face.  An
% open side spelt `route` names no direction of its own and answers nothing.
\cs_new_protected:Npn \__tenkz_kernel_r_label_open_face:nnn #1#2#3
  {
    ...
    \bool_lazy_and:nnT
      ...
      { ! \str_if_eq_p:Vn \l__tenkz_kernel_lab_face_tl {route} }
      {
...
\cs_new_protected:Npn \__tenkz_kernel_r_label_mark_toward:n #1
  {
    \int_compare:nNnTF
      ...
      {
        % On a circle the stations close into a ring, so the step from the
        % last column to the first runs east and not back across every
        % column between.  Numeric order reads that seam backwards and would
        % reserve the face opposite the ink, which is worse than reserving
        % none, so the seam answers nothing until the station is read in the
