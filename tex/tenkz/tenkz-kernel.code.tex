...
\cs_new_protected:Npn \__tenkz_kernel_addr_rel:nN #1#2
  {
    ...
    \__tenkz_kernel_node_put:NnV #2 {base} \l__tenkz_kernel_scratch_tl
  }
...
\cs_new_protected:Npn \__tenkz_kernel_addr_two:nnN #1#2#3
  {
    ...
    \__tenkz_kernel_node_put:NnV #3 {second} \l__tenkz_kernel_scratch_tl
  }
    ...
    \str_if_eq:VnF \l_tmpa_tl {cell}
      {
        \msg_error:nneee {tenkz}{kernel-address-operand}
          { \__tenkz_kernel_node_item:Nn \l__tenkz_kernel_scratch_tl {text} }
          {#1} { a~cell }
        \msg_line_context:
      }
    \__tenkz_kernel_node_put:NnV #2 {cell} \l__tenkz_kernel_scratch_tl
  }
...
\cs_new_protected:Npn \__tenkz_kernel_addr_operand:nN #1#2
  {
    \tl_set:Nn #2 {#1}
    \tl_trim_spaces:N #2
    \exp_args:NV \tl_if_head_is_group:nTF #2
      { \tl_set:Ne #2 { \exp_args:NV \tl_head:n #2 } }
      { }
...
\keys_define:nn { tenkz-kernel-picture }
  {
    sandwich .meta:n = { rows = {ket,op,bra} },
    sandwich .default:n = ,
...
\__tenkz_kernel_value:nnn   { tenkz-kernel-wire } { weight } { weight }
...
% globally; \tn and the environments rebind per group through \tenkzkernel
% until the S4 swap deletes both the switch and the 0.7 bindings.

% a closed wire takes no positional ends, so the ends are peeked, not
% declared: two brace groups follow or none do.
\NewDocumentCommand \tnwire { O{} }
  {
    \peek_remove_spaces:n
      {
        \peek_catcode:NTF \c_group_begin_token
          { \__tenkz_kernel_tnwire:nnn {#1} }
          { \__tenkz_kernel_tnwire:nnn {#1} { } { } }
      }
  }
...
\NewDocumentCommand \tenkzkernel { }
  {
    \RenewDocumentEnvironment { tenkz } { O{} }
      { \__tenkz_kernel_begin:n {##1} }
      { \__tenkz_kernel_end: }
    \RenewDocumentEnvironment { tenkzeq } { O{} }
      { \__tenkz_kernel_eq_begin:n {##1} }
      { \__tenkz_kernel_eq_end: }
