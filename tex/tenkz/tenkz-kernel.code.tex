...
\msg_new:nnn {tenkz}{kernel-skin-pairing-port-alias}
  { [TKZ-SKIN-PAIRING-PORT-ALIAS]~declared~skin~'#1'~uses~distinct~
    bearings~#2~and~#3~for~cell~port~#4;~one~cell~lane~cannot~represent~both. }
...
\prop_new:N \l__tenkz_kernel_skin_pairing_cell_port_prop
...
\cs_new_protected:Npn \__tenkz_kernel_port_used_collect:
  {
    ...
    % `physical=` owns the transverse centre identity of an odd-width host.
    % Registration refines every existing endpoint but consumes nothing; an
    % even-width centre lies between numbered slots and owns no identity.
    \__tenkz_model_map_ids:nn {atom}
      {
        \__tenkz_kernel_policy_leg_port_slot:nN {##1}
          \l__tenkz_kernel_port_slot_tl
...
\cs_new_protected:Npn
  \__tenkz_kernel_skin_pairing_cell_port_claim:nnnn #1#2#3#4
  {
    \__tenkz_kernel_bearing_normalize:nN {#2}
      \l__tenkz_kernel_r_b_tl
    \tl_set:Ne \l__tenkz_kernel_scratch_tl
      { #1 / #3 / \int_eval:n {#4} }
    \prop_get:NVN \l__tenkz_kernel_skin_pairing_cell_port_prop
      \l__tenkz_kernel_scratch_tl \l__tenkz_kernel_r_c_tl
    \quark_if_no_value:NTF \l__tenkz_kernel_r_c_tl
      {
        \prop_put:NVV \l__tenkz_kernel_skin_pairing_cell_port_prop
          \l__tenkz_kernel_scratch_tl \l__tenkz_kernel_r_b_tl
      }
      {
        \fp_compare:nNnF
          { \l__tenkz_kernel_r_c_tl } = { \l__tenkz_kernel_r_b_tl }
          {
            \msg_error:nneeee {tenkz}{kernel-skin-pairing-port-alias}
              { \tl_use:N \l__tenkz_kernel_r_skin_declared_tl }
              { \tl_use:N \l__tenkz_kernel_r_c_tl }
              { \tl_use:N \l__tenkz_kernel_r_b_tl }
              { #3@\int_eval:n {#4} }
...
\cs_new_protected:Npn \__tenkz_kernel_skin_pairing_end:nnnn #1#2#3#4
  {
    ...
    \bool_if:NT \l_tmpa_bool
      {
        \__tenkz_kernel_skin_pairing_cell_port_claim:nnnn
          {#2}
          { \tl_use:N \l__tenkz_kernel_r_face_angle_tl }
...
\cs_new_protected:Npn \__tenkz_kernel_begin:n #1
  {
    ...
    \prop_clear:N \l__tenkz_kernel_skin_pairing_cell_port_prop
