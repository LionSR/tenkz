...
\cs_new_protected:Npn \__tenkz_kernel_r_atom_glyph_style:nnN #1#2#3
  {
    \__tenkz_kernel_r_span:nNN {#1}
      \l__tenkz_kernel_r_row_tl \l__tenkz_kernel_r_col_tl
    \tl_set:Nn \l__tenkz_kernel_r_skin_base_tl {#2}
    \prop_get:NnNT \g__tenkz_kernel_skin_prop {#2}
      \l__tenkz_kernel_r_skin_descriptor_tl
      {
        \exp_args:NnV \regex_extract_once:nnNT
          { (?: \A | , ) \s* base \s* = \s* ([A-Za-z][A-Za-z0-9\-]*) }
          \l__tenkz_kernel_r_skin_descriptor_tl
          \l__tenkz_kernel_match_seq
          {
            \tl_set:Ne \l__tenkz_kernel_r_skin_base_tl
              { \seq_item:Nn \l__tenkz_kernel_match_seq {2} }
          }
      }
...
% A declared skin's pairings are wires between its own ports.  They ink after
% the silhouette, with the established paper halo, so the ordered stroke is
% legible both over the glyph outline and where two pairings approach.
\cs_new_protected:Npn \__tenkz_kernel_r_skin_pairings:nn #1#2
  {
    \prop_get:NnNT \g__tenkz_kernel_skin_prop {#2}
      \l__tenkz_kernel_r_skin_descriptor_tl
      {
        \exp_args:NnV \regex_extract_once:nnNT
          ...
          \l__tenkz_kernel_match_seq
          {
            ...
            \exp_args:NV \clist_map_inline:nn
              ...
              { \__tenkz_kernel_r_skin_pairing:nn {#1} {##1} }
          }
      }
  }
\cs_new_protected:Npn \__tenkz_kernel_r_skin_pairing:nn #1#2
  {
    \regex_extract_once:nnNTF
      {
        \A \s*
      ...
      {#2} \l__tenkz_kernel_match_seq
      {
        \bool_lazy_and:nnTF
          ...
          {
            \int_compare_p:nNn
              { \seq_item:Nn \l__tenkz_kernel_match_seq {5} } = {1}
          }
          {
            \tl_set:Ne \l__tenkz_kernel_r_skin_from_tl
              { \seq_item:Nn \l__tenkz_kernel_match_seq {2} }
            \tl_set:Ne \l__tenkz_kernel_r_skin_to_tl
              { \seq_item:Nn \l__tenkz_kernel_match_seq {4} }
            ...
            \exp_args:NV \__tenkz_kernel_r_species_hue:nN
              ...
              \l__tenkz_kernel_r_hue_tl
            \exp_args:NV \__tenkz_render_skin_pairing:nn
              \l__tenkz_kernel_r_hue_tl
              {
                (
                  #1.
                ...
                to
                  [
                    out =
                      \fp_eval:n
                        {
                          \l__tenkz_kernel_r_skin_from_tl - 180
                          + \l__tenkz_kernel_r_ink_turn_tl
                        } ,
                    in =
                      \fp_eval:n
                        {
                          \l__tenkz_kernel_r_skin_to_tl + 180
                          + \l__tenkz_kernel_r_ink_turn_tl
                        }
                  ]
                (
          ...
          {
            \msg_error:nnee {tenkz}{kernel-render-todo}
              { skin~pairing~slot~other~than~1 } {#1}
          }
      }
      {
        \msg_error:nnee {tenkz}{kernel-render-todo}
...
\cs_new_protected:Npn \__tenkz_kernel_r_atom_glyph:n #1
  {
    ...
    \exp_args:NnV \__tenkz_kernel_r_skin_pairings:nn
      {#1} \l__tenkz_kernel_r_tl
  }
