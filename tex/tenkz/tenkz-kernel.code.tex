                ...
                \bool_if:NTF \l__tenkz_kernel_lab_far_bool
                  {
                    ...
                    % under one glyph and leaves it by no face at all.
                    \__tenkz_kernel_any_cell_claim:eeN
    ...
    \quark_if_no_value:NTF \l__tenkz_kernel_lab_face_tl
      {
        ...
        \str_if_eq:VnT \l__tenkz_kernel_lab_node_tl {string}
          { \__tenkz_model_get:nnN {#1} {via} \l__tenkz_kernel_lab_via_tl }
        % the cell reading stands down on a circle for a carrier exactly as
        % it does for an endpoint, and for the same reason
        \bool_lazy_and:nnT
...
% #3 says which end is open, so it says whether the word points along the
% route or back down it, and #4 is the word.
