...
\tl_new:N   \l__tenkz_kernel_check_spec_tl
...
% An unstated size class resolves from the surrounding mathematics; a stated
% one stands.  The resolved class enters the record either way, so a reader
% of the stream never has to guess which one acted.
...
\cs_new_protected:Npn \__tenkz_kernel_eq_begin:n #1
  {
    ...
    \quark_if_no_value:NF \l__tenkz_kernel_scratch_tl
      {
        % The options are read from the specification with the opt-out
        % payloads taken out first: a reason is prose an author writes about
        % the mathematics, and prose that happens to name an option is still
        % prose.  What survives that removal must be one of the words the
        % audit knows, or the author has written something the audit will
        % not do and believes it will.
        \tl_set_eq:NN \l__tenkz_kernel_check_spec_tl \l__tenkz_kernel_scratch_tl
        \regex_replace_all:nnN
          {
            off \s* = \s* \{ \s* \d+ \s* : \s*
              (?: [^\{\}] | \{ [^\{\}]* \} )* \}
          }
          { } \l__tenkz_kernel_check_spec_tl
        \exp_args:NnV \regex_match:nnT
          { modulo \s* = \s* bundles }
          \l__tenkz_kernel_check_spec_tl
          { \bool_set_true:N \l__tenkz_kernel_check_bundles_bool }
        \regex_replace_all:nnN
          { signature | modulo \s* = \s* bundles | [\s,] }
          { } \l__tenkz_kernel_check_spec_tl
        \tl_if_empty:NF \l__tenkz_kernel_check_spec_tl
          {
            \msg_error:nne {tenkz}{kernel-check-word}
              { \tl_use:N \l__tenkz_kernel_check_spec_tl }
          }
        \exp_args:NnV \regex_extract_all:nnN
          {
            off \s* = \s* \{ \s* (\d+) \s* : \s*
        ...
        \int_step_inline:nnnn {2} {3}
          ...
          {
            % An opt-out without a reason is not an opt-out: what the key
            % records is the mathematics the diagram does not draw, and a
            % blank reason records nothing while switching the comparison
            % off.  The relation stays audited and the author is told.
            \tl_set:Ne \l__tenkz_kernel_scratch_tl
              { \seq_item:Nn \l__tenkz_kernel_match_seq { ##1 + 1 } }
            \tl_trim_spaces:N \l__tenkz_kernel_scratch_tl
                ...
                \__tenkz_kernel_event:n
                  {
                    ...
                    |result=off
                    |reason=\tl_use:N \l__tenkz_kernel_scratch_tl
                  }
              }
          }
      }
    \prop_remove:Nn \l__tenkz_kernel_stage_prop {check}
    % One metric context: the equation senses its class once, here, where it
    % can still see the mathematics it stands in, and every panel inherits
    % it.  A panel that is fenced or braced sits in inner mathematics of its
    % own and would otherwise sense a denser class than its partner.
    \__tenkz_kernel_size_resolve:N \l__tenkz_kernel_stage_prop
    \prop_set_eq:NN
      \l__tenkz_kernel_eq_policy_prop \l__tenkz_kernel_stage_prop
    \prop_clear:N \l__tenkz_kernel_stage_prop
