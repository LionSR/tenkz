...
%
% The words come from the registry's tombstone rows and from nowhere else
% (LANGUAGE-1.0 section 10).  The kernel owns when a refusal is installed; the
% registry owns which spellings are dead and what each one migrates to, so the
% parser, the linter and the generated reference read one record.
...
% One registry row.  A row that names no value retires a whole key or command,
% which the linter reports and the parser has already stopped installing; only
% a row naming a value of a live kernel key becomes a refusal here.
\cs_new_protected:Npn \__tenkz_kernel_tombstone_row:nnnnn #1#2#3#4#5
  {
    \tl_if_empty:nF {#3}
      {
        \str_if_eq:eeT { \str_range:nnn {#1} {1} {7} } { kernel- }
          {
            \tl_if_empty:nTF {#4}
              { \__tenkz_kernel_tombstone:nnnn { tenkz-#1 } {#2} {#3} {#5} }
              { \__tenkz_kernel_tombstone:nnnn { tenkz-#1 } {#2} {#3}
                  { use~#4 . ~ #5 } }
          }
      }
  }
\cs_new_protected:Npn \__tenkz_kernel_tombstones:
  {
    \seq_map_inline:Nn \g__tenkz_language_tombstones_seq
      { \__tenkz_kernel_tombstone_row:nnnnn ##1 }
  }
...
% Last, so that a tombstone overrides the alphabet's own unknown branch for
% every key tree above: the registry's dead spellings become refusals that
% name their migration.
\__tenkz_kernel_tombstones:
