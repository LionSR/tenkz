...
\cs_new_protected:Npn \__tenkz_kernel_r_route_leg:nn #1#2
  {
    ...
    % the lane travels with the axes it is a coordinate in, because the leg
    % that must reach it grows out of an atom which stands in axes of its own
    ...
    \quark_if_no_value:NF \l__tenkz_kernel_ro_reach_tl
      {
        \fp_compare:nNnT
          { abs( \tl_item:Nn \l__tenkz_kernel_ro_reach_tl {1} ) }
          > { abs( \l__tenkz_kernel_ro_lane_tl ) }
          {
            \tl_set_eq:NN
              \l__tenkz_kernel_ro_row_tl \l__tenkz_kernel_ro_reach_tl
          }
      }
