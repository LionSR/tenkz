...
% alphabet error alone cannot do that -- it reports the words that remain,
% which reads as a typo when the word was in fact retired.  A tombstone is a
...
\__tenkz_kernel_choice:nnnn { tenkz-kernel-mark } { form }
  ...
  { form }
% The four spellings the alphabet lost.  Each accepted a record and drew
% nothing, and each has one live word that says the same thing, so the parser
% keeps the dead spelling long enough to name its successor rather than
% answering with the alphabet and leaving a reader to guess whether the word
% was mistyped or removed.
