...
\prop_new:N \l__tenkz_kernel_eq_saved_skin_prop
\prop_new:N \l__tenkz_kernel_eq_saved_species_prop
...
\seq_new:N  \g__tenkz_kernel_eq_declared_gseq
...
\cs_new_protected:Npn \__tenkz_kernel_declare_atom_aux:nn #1#2
  {
    % A panel measured for an equation may declare an atom of its own, and
    % the declaration is global.  The measuring run notes what it minted so
    % the real run can mint it again from an undefined name, which is what
    % the declaration asks of the document and what the author wrote once.
    \bool_if:NT \l__tenkz_kernel_eq_probe_bool
      { \seq_gput_right:Nn \g__tenkz_kernel_eq_declared_gseq {#1} }
...
\cs_new_protected:Npn \__tenkz_kernel_eq_measure:
  {
    \group_begin:
      ...
      \prop_set_eq:NN \l__tenkz_kernel_eq_saved_skin_prop
        \g__tenkz_kernel_skin_prop
      \prop_set_eq:NN \l__tenkz_kernel_eq_saved_species_prop
        \g__tenkz_kernel_species_prop
      \seq_gclear:N \g__tenkz_kernel_eq_declared_gseq
      ...
      \prop_gset_eq:NN \g__tenkz_kernel_skin_prop
        \l__tenkz_kernel_eq_saved_skin_prop
      \prop_gset_eq:NN \g__tenkz_kernel_species_prop
        \l__tenkz_kernel_eq_saved_species_prop
      \seq_map_inline:Nn \g__tenkz_kernel_eq_declared_gseq
        { \cs_undefine:c {##1} }
      \seq_gclear:N \g__tenkz_kernel_eq_declared_gseq
      \seq_gclear:N \g__tenkz_kernel_panel_gseq
      \seq_gclear:N \g__tenkz_kernel_panel_kind_gseq
