...
\msg_new:nnnn {tenkz}{cross-bond-undeclared}
  { [TKZ-CROSS-BOND]~author~string~'#1'~crosses~lattice~bond~'#2'~with~
    no~declared~crossing }
...
% A renderer-owned touch-all route (e.g. a lattice bond) silences the
% ordinary crossing police against every other route, which is right for
% the topology it owns but wrong for a route it does not: an author string
% is never touch-all's to silence.  If exactly one side of this pair is an
    ...
    \prop_if_in:NnT \l__tenkz_string_author_prop {#1}
      {
        \prop_if_in:NnT \l__tenkz_string_touch_all_prop {#2}
          {
            \bool_set_true:N \l_tmpb_bool
            \__tenkz_string_police_pair_bond:nn {#1} {#2}
          }
      }
    \bool_if:NF \l_tmpb_bool
      {
        \prop_if_in:NnT \l__tenkz_string_author_prop {#2}
          {
            \prop_if_in:NnT \l__tenkz_string_touch_all_prop {#1}
              { \__tenkz_string_police_pair_bond:nn {#2} {#1} }
          }
      }
  }
% #1 is an author-declared string, #2 the touch-all route (lattice bond) it
% was found to meet with no declared order.  A geometric meeting here is
% not the author's to leave unbroken: cutting the calibrated break into the
% string -- so the bond's own ink stays whole and the string reads as
% passing behind it -- and warning is issue #5825's chosen middle ground;
% turning every such silent overlap into a hard error is future work the
% issue explicitly defers.
\cs_new_protected:Npn \__tenkz_string_police_pair_bond:nn #1#2
  {
    \__tenkz_string_pair_count_get:NnnN
      \l__tenkz_string_cross_prop {#1} {#2}
      \l__tenkz_string_declared_int
    \int_compare:nNnT { \l__tenkz_string_declared_int } = {0}
      {
        ...
        \int_compare:nNnT { \l__tenkz_string_n_int } > {0}
          {
            \msg_warning:nnnn {tenkz}{cross-bond-undeclared} {#1} {#2}
